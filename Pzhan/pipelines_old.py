...
class PzhanPipeline(object):
    ...
    def process_item(self, item, spider):
        if spider.name == 'pp':
            ...
            for i in range(0, len(oLinks)):
                fname = re.sub("\D\W", "", item["name"])
                fartist = re.sub("\D\W", "", item["artist"])
                if fname.strip() == '':  # 如果全都是非法字符构成的
                    fname = pid  # 就用ID代替名字吧
                if fartist.strip() == '':  # 同上
                    fartist = pid  # 就用ID代替名字吧
                path = 'D:/spiderResultPzhan/'
                self.downLoadPic(url=oLinks[i], name=fname + str(i + 1), size=trueSize, artist=fartist,
                                 referer=item["referer"], pid=pid, path=path)

        if spider.name in ['art', 'all']:
            oLinks = []
            trueSize = ''
            tags = ''
            for i in item['tags']:
                tags += '_' + validate_title(i)
            # 获取关键部分
            pUrl = item["pLink"]
            reg = r'.+/(\d+)_p'
            pid = re.findall(reg, pUrl)[0]
            ...
            for i in range(0, len(oLinks)):
                # fname = re.sub("\D\W", "", item["name"])
                # fartist = re.sub("\D\W", "", item["artist"])
                fname = validate_title(item["name"])
                fartist = validate_title(item["artist"])
                if fname.strip() == '':  # 如果全都是非法字符构成的
                    fname = pid  # 就用ID代替名字吧
                if fartist.strip() == '':  # 同上
                    fartist = pid  # 就用ID代替名字吧
                pool = ThreadPoolExecutor(5)
                pool.submit(self.artDownLoadPic, url=oLinks[i], name=fname + str(i + 1), size=trueSize, artist=fartist,
